...
const ADVISOR_PROMPT = (appTxt, totalWh, totalW, prefs, priceContext) => `
...
let AdvisorService = AdvisorService_1 = class AdvisorService {
    constructor(sessionRepo, productRepo, cfg) {
        this.sessionRepo = sessionRepo;
        this.productRepo = productRepo;
        this.cfg = cfg;
        this.logger = new common_1.Logger(AdvisorService_1.name);
        this.anthropic = new sdk_1.default({ apiKey: cfg.get('anthropic.apiKey') });
    }
    async calculate(appliances, preferences, userId) {
        const totalW = appliances.reduce((s, a) => s + a.watts * a.quantity, 0);
        const totalWh = appliances.reduce((s, a) => s + a.watts * a.quantity * (a.hoursPerDay || 8), 0);
    ...
    async getMarketplaceItemsForSession(sessionId, tier, preference = 'balanced') {
        const session = await this.sessionRepo.findOne({ where: { id: sessionId } });
        if (!session?.results)
            throw new common_1.NotFoundException('Session not found');
        const rec = session.results.recommendations?.find((r) => r.id === tier);
        if (!rec)
            throw new common_1.NotFoundException(`Recommendation "${tier}" not found`);
        const panelW = rec.panels?.wattageEach || 400;
        const battType = rec.batteries?.type || '';
        ...
        const fetch = (q, limit = 6) => this.buildProductQuery(q, preference).take(limit).getMany();
        ...
        const fmt = (products) => products.map(p => ({
            id: p.id,
            name: p.name,
            slug: p.slug,
        ...
        for (const { label, terms } of queries) {
            try {
                ...
                const products = await this.productRepo.createQueryBuilder('p')
                    ...
                    .andWhere('p.stock > 0')
                    .andWhere('(' + terms.map((_, i) => `p.name ILIKE :t${i}`).join(' OR ') + ')', params)
                    .select(['p.price'])
                    .take(40)
                    .getMany();
                if (!products.length)
    ...
    buildProductQuery(search, preference) {
    ...
    async saveSelection(sessionId, recommendationId) {
        await this.sessionRepo.update(sessionId, { selectedRecommendation: recommendationId });
    }
    fallbackCalc(appliances, totalW, totalWh, prefs) {
...
exports.AdvisorService = AdvisorService = AdvisorService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(advisor_entity_1.AdvisorSession)),
